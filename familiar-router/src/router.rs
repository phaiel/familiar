...
use crate::{RoutingContext, RoutingDecision, RoutingTrace, RoutingTable, RouterError, Result};
...
pub struct Router {
    /// Pre-compiled routing table generated from schemas
...
impl Router {
    ...
    async fn evaluate_policies<'a>(
        ...
        for (node_id, node_info) in &context.nodes {
    ...
    fn generate_fallbacks(
        ...
        candidates.iter().take(3).filter_map(|(node_id, score)| {
            context.nodes.get(node_id).map(|node| {
