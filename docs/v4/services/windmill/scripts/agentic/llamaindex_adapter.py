...
# ---------------------------------------------------------------------------
# Weave Schema Types - Generated from familiar-schemas JSON Schema
#
# SCHEMA-FIRST ARCHITECTURE:
# These types MUST match the canonical definitions in:
#   - familiar-schemas/versions/latest/json-schema/components/WeaveUnit.schema.json
#   - familiar-schemas/versions/latest/json-schema/tools/SpawnSuggestion.schema.json
#   - familiar-schemas/versions/latest/json-schema/tools/SpawnSummary.schema.json
#
# To regenerate, run: cargo xtask schemas sync --pydantic
# This will use datamodel-codegen to generate from JSON Schema.
# ---------------------------------------------------------------------------

# Try to import generated types, fall back to local definitions if not generated yet
try:
    from generated_pydantic.components import WeaveUnit, WeavePhysics, WeaveEntity
    from generated_pydantic.tools import SpawnSuggestion, SpawnSummary, SpawnResult, WeaveResult
    USING_GENERATED_WEAVE = True
except ImportError:
    USING_GENERATED_WEAVE = False
    
    # TEMPORARY: Fallback definitions until generated types are available
    # DO NOT MODIFY - these must stay in sync with JSON Schema definitions
    
    class WeavePhysics(BaseModel):
        """Physics dimensions - MUST match familiar-schemas WeavePhysics"""
        valence: float = 0.0
        arousal: float = 0.0
    ...
    class WeaveEntity(BaseModel):
        """Entity classification - MUST match familiar-schemas"""
        type: str
        probability: float

    class WeaveUnit(BaseModel):
        """A single unit of woven content - MUST match components/WeaveUnit.schema.json"""
        index: int
        content: str
    ...
    class SpawnSuggestion(BaseModel):
        """Spawn suggestion - MUST match tools/SpawnSuggestion.schema.json"""
        action: str = "suggest"
        reason: str
        content: str
        physics: WeavePhysics
