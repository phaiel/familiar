...
export async function main(user_id: string, user_email: string, tenant_id: string, tenant_name: string, role: string) {
  const metadataJson = JSON.stringify({ tenant_name, role, via: 'code' });
  ...
  if (code !== 0) {
    ...
    // Don't throw - audit logging failure shouldn't block invitation acceptance
  }
